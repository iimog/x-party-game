...
public class Spielablauf implements GameListener, Ablauf {	
	...
	public Spielablauf(Player[] pl, int numOfGames, List<Integer> gameList, Modus modus, Profile profile){
		this.modus = modus;
		spielerZahl = modus.getSpielerzahl();
		if(modus == Modus.SOLO)spielerZahl++;
		this.myPlayer = pl;
	...
	public static Spielablauf createSpielablauf(Player[] pl, ArrayList<Integer> gameList, Modus modus){
		...
		if(modus == Modus.SOLO)sa.spielerZahl++;
		return sa;
	}
	/**
	...
	public void gameOver() {
		...
		for(int i=0; i<actGame.myPlayer.length; i++){
			...
			for(int j=0; j<actGame.myPlayer.length; j++){
				if(i!=j && actGame.myPlayer[j].gameCredit >= actGame.myPlayer[i].gameCredit){
		...
		if(winnerID != -1){
			...
			if(modus != Modus.TEAM)
				HighscoreFileHandler.saveGameHighscore(new Ergebnis(actGame, myPlayer[winnerID]));
		}
		if(winnerID == -1 && actGame.myPlayer[0].gameCredit == MatchCredits.UNENTSCHIEDEN){
		...
		// TODO eine newGame() oder reset() Methode für Player schreiben, wo alle Spielwerte zurückgesetzt werden
	...
	private void matchBeenden() {
		...
		HighscoreFileHandler.saveMatchHighscore(modus, myPlayer, mCredits.getWinner());
		X.getInstance().changeAnzeige(new Siegerehrung(mCredits.getWinner()));
	}
	/**
	 * Diese Methode liefert die ID des nächsten Spiels durch Zufall.
	 * Falls wiederholen false ist dürfen sich die Spiele erst wiederholen, wenn alle einmal durch sind.
	 * @param wiederholen
	 * @return nextID
	 */
		...
		while (true) {
			...
			if (!wiederholen) {
				if (playedGameIDs.add(nextID))
					...
					if(playedGameIDs.size()>=getErwuenschteSpieleIDs().size()-1) // -1 da Zufall ein Element ist
	...
	public void startNextGame(){
		...
		}catch(IndexOutOfBoundsException e){		// Zufallsspiel falls die GameListe verbraucht ist
